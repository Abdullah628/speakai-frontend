...
interface UserStats {
  ...
  achievements?: { name: string; icon: string; description: string; earned: boolean }[];
  recentActivity?: [{ date: string; activity: string; accuracy: number; type: string }];
}
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-8">
          <Link href="/chat">
            <Card className="hover:shadow-lg transition-shadow cursor-pointer">
              ...
              <CardContent>
                <CardDescription>
                  Begin a new conversation with your AI tutor
                </CardDescription>
              </CardContent>
            </Card>
          </Link>
          ...
          <Card className="hover:shadow-lg transition-shadow">
            ...
            <CardContent>
              <CardDescription>
                Check your learning analytics and improvements
              </CardDescription>
            </CardContent>
          </Card>
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm text-gray-600">
                  {stats.nextLevelProgress}% to next level
                </span>
              </div>
              <Progress value={stats.nextLevelProgress || 65} className="h-3" />
              <p className="text-sm text-gray-600">
                Complete more practice sessions to reach the next level!
              </p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-2 gap-4">
                {stats?.achievements
                  ?.filter((achievement) => achievement.earned !== false)
                  .map((achievement, index) => {
                    const Icon = iconMap[achievement.icon]; // Capitalized so JSX knows it's a component
                    return (
                      <div
                        key={index}
